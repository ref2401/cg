...
void main()
{
	...
	if (all(equal(vert_tex_coord, vec2(0, 0))))
		vs_out.view_ray_vs = u_arr_far_plane_coords[0];

	// right-bottom
	if (all(equal(vert_tex_coord, vec2(1, 0))))
		vs_out.view_ray_vs = u_arr_far_plane_coords[1];

	// right-top
	if (all(equal(vert_tex_coord, vec2(1, 1))))
		vs_out.view_ray_vs = u_arr_far_plane_coords[2];

	// left-top
	if (all(equal(vert_tex_coord, vec2(0, 1))))
		vs_out.view_ray_vs = u_arr_far_plane_coords[3];
}